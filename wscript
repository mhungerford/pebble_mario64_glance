...
def gifs_to_apngs(gifs_dir, appinfo_filename, resources_dir):
    ...
    for gif in gifs:
        if '.gif' in gif:
            ...
            subprocess.call('gifsicle --resize-fit 144x124 ' + 
            '--resize-method lanczos3 ' + '--colors 64 -O1 -o ' + resources_dir + '/' + 
            gif_mod + ' ' + gifs_dir + '/' + gif, shell=True)
            subprocess.call('gif2apng -z0 ' + resources_dir + '/' + gif_mod + ' ' + 
...
def configure(ctx):
    # DISABLED!  Re-enable to regenerate apng from gif, not needed for source modifications
