...
// Glancing data
static GlanceState state = GLANCING_ACTIVE;  // default to active for initial frame
...
static void load_sequence(uint32_t resource_id) {
  ...
  image_sequence_timer = app_timer_register(0, sequence_update, NULL);
}
...
void glancing_callback(GlancingData *data) {
  ...
  switch (state) {
    case GLANCING_ACTIVE:
      // schedule timer for animation sequence
...
static void window_load(Window *window) {
  ...
  int time_yoffset = DISP_SHAPE_SELECT(136, 130);
  int date_yoffset = DISP_SHAPE_SELECT(0, 4);
  int bitmap_yoffset = DISP_SHAPE_SELECT(-4, -10);
  int bitmap_xoffset = DISP_SHAPE_SELECT(4, 4);
  ...
  sequence_update(NULL);  // force load first frame
  state = GLANCING_INACTIVE; // disable animation

  //use glancing
  glancing_service_subscribe(10 * 1000, true, glancing_callback);
}
